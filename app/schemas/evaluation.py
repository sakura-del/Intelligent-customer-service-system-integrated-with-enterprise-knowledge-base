...
class TunerParamsUpdateRequest(BaseModel):
    ...
    vector_top_k: Optional[int] = Field(
        None, description="向量召回数量，建议范围 20-30"
    )
    bm25_top_k: Optional[int] = Field(
        None, description="BM25 召回数量，建议范围 20-30"
    )
    rerank_top_k: Optional[int] = Field(
        None, description="重排序后保留数量，建议范围 3-5"
    )
    similarity_threshold: Optional[float] = Field(
        None, description="相似度阈值，建议范围 0.6-0.7"
    )
    rrf_k: Optional[int] = Field(
        None, description="RRF 平滑常数，建议范围 40-80"
    )
    rrf_vector_weight: Optional[float] = Field(
        None, description="RRF 向量路权重，建议 0.6"
    )
    rrf_keyword_weight: Optional[float] = Field(
        None, description="RRF 关键词路权重，建议 0.4"
    )
...
class EvaluationRunRequest(BaseModel):
    ...
    top_k: Optional[int] = Field(
        None, ge=1, le=50, description="评测时检索 Top-K，为空时使用调优参数"
    )
...
class ReportSummary(BaseModel):
    ...
    source: str = Field(
        "default", description="测试集来源：default=内置 / external=外部"
    )
