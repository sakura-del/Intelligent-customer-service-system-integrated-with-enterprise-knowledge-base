...
def list_alerts(
    level: str | None = Query(None, description="按级别过滤：info/warn/error/critical"),
    source: str | None = Query(None, description="按来源过滤，如 token_usage"),
    since: str | None = Query(None, description="起始时间（ISO8601），仅返回该时间之后的告警"),
) -> list[dict[str, Any]]:
    """查询告警列表。
    ...
    alerts = get_alert_manager().list_alerts(level=level_enum, source=source, since=since)
    return [alert.model_dump(mode="json") for alert in alerts]


def _parse_alert_level(level: str | None) -> AlertLevel | None:
...
def token_usage(
    window: str = Query("hour", description="统计窗口：minute/hour/day"),
) -> dict[str, Any]:
    """返回指定窗口的 Token 用量统计。
