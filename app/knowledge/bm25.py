...
class BM25Retriever:
    ...
    def index(self, chunks: list[TextChunk], ids: list[str] | None = None) -> int:
        """构建 BM25 索引，返回索引条目数。
        ...
        if self._use_lib:
            try:
                ...
                logger.info("BM25 索引构建完成（rank-bm25）：条目数=%d", len(self._chunk_ids))
            except Exception as exc:
                # 库版本/数据异常时降级，保证链路不中断
                logger.warning("rank-bm25 初始化失败，降级到自实现 BM25：%s", exc)
                self._use_lib = False
                self._bm25 = _SimpleBM25(self._tokenized)
        else:
            self._bm25 = _SimpleBM25(self._tokenized)
            logger.info("BM25 索引构建完成（自实现 fallback）：条目数=%d", len(self._chunk_ids))
